...
let rec removeZero l = match l with
  ...
  | h::h2::t  ->  
      if h = 0 
      then if h2 != 0 then h2::t else removeZero t
      else l
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = 
      let (carry, finalResult) = a in 
      let (b1, b2) = x in 
        ((b1+b2+carry)/10, ((b1+b2+carry) mod 10)::finalResult) in
    let base = 0,[] in
...
let _ = bigAdd [9;9;9;9] [0;-9;-9;-9;-9];; 
...
let bigMul l1 l2 = 
  let f a x = 
    ...
    let (i, i2)   = x in
    let (b1, b2)  = a1 in 
      (0::b1, b2), (bigAdd ((mulByDigit i b2)@b1) a2) in
  let base = ([],l2), [] in
...
let _ = bigMul [0;2;2;2;2] [9;9;9;9;9] 
let _ = bigMul [1] [9] 
let _ = bigMul [] [] 
