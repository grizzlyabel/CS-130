...
let rec digitsOfInt n = 
  let rec dig x y =
    if (y <= 0) then [] else
    if y < 10 then y::x else dig((y mod 10)::x) (y/10) in dig [] n 

(*uncomment and run AFTER you have implemented digitsOfInt *)

let _ = digitsOfInt (-3124)
let _ = digitsOfInt 352663
let _ = digitsOfInt (20012)
...
let _ = additivePersistence (-66)
let _ = additivePersistence (-6932)
...
let palindrome w = 
  match w with
    | "" -> true
    | _ -> if (explode w) = listReverse(explode w) then true else false
