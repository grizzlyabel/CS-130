...
let _ = removeDuplicates [-1;6;2;4;12;12;12;12;1];;
let _ = removeDuplicates [2];;
...
let fixpoint (f,b) = 
  wwhile ((let gFunct b' = let temp = f b' in
               if temp = b' 
               then (b', false) 
               else (temp, true) 
           in gFunct), b)
...
type expr = 
    ...
    | Circle   of expr * expr
    | Tan      of expr 
    (*| Averages of expr * expr * expr*)
    | Whatever of expr * expr * expr
...
let rec exprToString e = 
  match e with
    ...
    | Circle (x, y)		-> "pi*("^exprToString x^"*"^exprToString y^")"
    | Tan e		        -> "tan("^exprToString e^")"
    (*| Averages (a, b, c)  -> "[("^exprToString a^"+"^exprToString b^"+"^exprToString c^")/2]" *)
    | Whatever (a, b, c)    -> "[-1*(sin("^exprToString a^"+"^exprToString b^"+"^exprToString c^"))/2]"
...
let buildCircle(e1, e2)		   = Circle(e1, e2)
let buildTan(e)		           = Tan(e)
(*let buildAverages(e1, e2, e3)      = Averages(e1, e2, e3)*)
let buildWhatever(e1, e2, e3)      = Whatever(e1, e2, e3)
...
let rec eval (e,x,y) = 
  match e with
    ...
    | Circle (expr1, expr2) -> (pi*.(eval(expr1, x, y)/.2.0)*.(eval(expr2, x, y)/.2.0))
    | Tan (e)		-> (tan(pi*.eval(e,x,y)))
    | Whatever (a, b, c)    -> (-1.0*.(cos(pi*.(sin(eval(a,x,y)+.eval(b,x,y)+.eval(c,x,y)))))/.2.0)
...
(* build: (int*int->int) * int -> Expr 
   ...
   The second argument is the depth, the first is a random function. 
   A call to rand(2,5) will give you a random number in the range [2,5].
...
let rec build (rand, depth) = 
  if depth = 0 then if rand(0, 1) = 1 then buildX() else buildY() 
  else if depth >= 9 then buildTimes(build(rand, depth-1), build(rand, depth-1))
  else
    let random = rand(0, 10) in
      match random with
        ...
        | 5 -> buildCircle(build(rand, depth-1), build(rand, depth-1))
        | 6 -> buildSine(build(rand, depth-1))(*Average(build(rand, depth-1), build(rand, depth-1))*)
        | 7 -> buildCosine(build(rand, depth-1))(*Thresh(build(rand, depth-1), build(rand, depth-1), build(rand, depth-1), build(rand, depth-1))*)
        | 8 -> buildWhatever(build(rand, depth-1), build(rand, depth-1), build(rand, depth-1))
        | 9 -> buildX()
        | _ -> buildY()

(*| 5 -> buildTan(build(rand, depth-1))*)
(*| _ -> buildAverages(build(rand, depth-1), build(rand, depth-1), build(rand, depth-1)) *)
